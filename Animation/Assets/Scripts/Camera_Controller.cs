...
public class Camera_Controller : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector3 desiredPosition = targetTransform.position - (rotation * offset);

        CompensateForWall(pivot.position, ref desiredPosition);

        transform.position = Vector3.Lerp(transform.position, desiredPosition + (rotation * offset * 0.1f), 0.1f);
